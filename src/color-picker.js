(function () {
  ...
  const palette =
    '<div class="' + paletteClass + '" style="display: none;"></div>';
  let paletteHasFocus = false;
  let desc = "Use a hex color code or use the tab key to select a color.";
  desc +=
    " Use the arrow keys to scroll through all colors. Use the space or return key to select the color.";
  ...
  function colorNavigation(e) {
    ...
    if (13 === e.which || 32 === e.which) {
      ...
      let colorSample = this.parentNode.parentNode.querySelector(
        "." + colorSampleClass
      );
      colorSample.style = "background-color: " + hexColor + ";";
      updateColorSampleText(colorSample, hexColor);
  ...
  function getHumanReadableColor(rgbColor, hexColor) {
    ...
    return (
      ...
      readable.hueName() +
      ", " +
      readable.saturationName() +
      ", " +
      readable.lightnessName()
    );
  }
